...
class DataSourceFactory:
    ...
    def create_random_population(self):
        ...
        active_man_in_total_population = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(2), size=1)[0])
        
        # print('test', active_man_int_total_population)
        active_people_in_working_age = np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(2), size=1).tolist()[0]
        print('active', active_people_in_working_age)
        working_age = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(2), size=1)[0])
        active_man_in_working_age = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(2), size=1)[0])
        active_woman_in_working_age = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(2), size=1)[0])
        active_by_sex = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(2), size=1)[0])
        proffesors_by_sex = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(2), size=1)[0])
        ...
        distribution_by_economic_activity = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(16), size=1)[0])
        man_distribution_by_economic_activity = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(16), size=1)[0])
        woman_distribution_by_economic_activity = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(16), size=1)[0])
        age_groups = [[0, 4], [5, 9], [10, 14], [15, 19], [20, 24], [25, 29], [30, 34], [35, 39], [40, 44], [45, 49], [50, 54], [55, 59], [60, 64], [65, 100]]
        distribution_by_age_groups = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(len(age_groups)), size=len(age_groups))[0])
        distribution_of_man_or_woman = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(2), size=1)[0])
        neighborhoods_per_thousand_people = 1.0
        inhabitants_distribution = list(np.random.dirichlet(alpha=self.create_random_alphas_for_dirichlet(9), size=1)[0])
...
x = DataSourceFactory().create_random_population()
print(x.dict())