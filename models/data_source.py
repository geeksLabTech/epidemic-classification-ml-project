...
class DataSourceFactory:
    ...
    def create_random_population(self):
        ...
        active_man_int_total_population = list(np.random.dirichlet(
            alpha=self.create_random_alphas_for_dirichlet(2), size=2)[0])
        active_people_in_working_age = list(np.random.dirichlet(
            alpha=self.create_random_alphas_for_dirichlet(2), size=2))
        ...
        distribution_by_economic_activity = list(np.random.dirichlet(
            alpha=self.create_random_alphas_for_dirichlet(16), size=16))
        ...
        distribution_by_age_groups = list(np.random.dirichlet(
            alpha=self.create_random_alphas_for_dirichlet(len(age_groups)), size=len(age_groups)))
        ...
        inhabitants_distribution = list(np.random.dirichlet(
            alpha=self.create_random_alphas_for_dirichlet(9), size=9))
    ...
    def create_random_alphas_for_dirichlet(self, size):
        return np.random.choice((1, size), size=size)
